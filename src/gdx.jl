...
mutable struct GDXHandle
    ...
    function GDXHandle()
        ...
        return new(cptr, Ref{Cint}(-1), Ref{Cint}(-1), civec, crvec, buf, cbuf)
    end
end
...
gdx_close(gdx_ptr::Ptr{Cvoid}) = ccall((:gdxclose, LIBGDX), Cint, (Ptr{Cvoid},), gdx_ptr)
