...
function MOI.add_constraint(
    ...
    model.variables[vi.value].lower_bound = max(0, min(1, ceil(model.variables[vi.value].lower_bound)))
    model.variables[vi.value].upper_bound = max(0, min(1, floor(model.variables[vi.value].upper_bound)))
...
function MOI.get(
    ...
    vi = MOI.VariableIndex(ci.value)
    check_inbounds(model, vi)
    if model.variables[vi.value].lower_bound == model.variables[vi.value].upper_bound && S == MOI.GreaterThan{Float64}
        return 0.0
    end
...
const SupportedScalarConstraintFunctions{T} = Union{
    MOI.ScalarAffineFunction{T},
    MOI.ScalarQuadraticFunction{T}
}

const SupportedScalarConstraintSets{T} = Union{
    MOI.LessThan{T},
    MOI.GreaterThan{T},
    MOI.EqualTo{T}
}
...
function MOI.set(
    ...
    check_inbounds(model, ci)
    model.compl_constraints[ci.value].name = value
end
...
function MOI.get(
    ...
    s = model.sense == MOI.MIN_SENSE ? 1.0 : -1.0
    for i = 1:length(model.nlp_data.constraint_bounds)
